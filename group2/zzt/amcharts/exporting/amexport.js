AmCharts.AmExport = AmCharts.Class({
	construct: function(chart, cfg) {
		...
		_this.cfg		= {
			...
			menuItems				: [{
				...
				iconTitle				: 'Save chart as an images',
				format					: 'png'
			}],
			menuItemStyle			: {
	...
	*/
	generateBinaryArray: function(base64_string) {
		...
		// doesn't really matter with most images formats.
	...
	*/
	generatePDF: function(cfg) {
		var _this = this,
			...
			data = _this.canvas.toDataURL('images/jpeg'), // JSPDF ONLY SUPPORTS JPG
	...
	/*
	Creates the CANVAS to receive the images data
	@param format void()
		...
		*/
		function internalCallback() {
			...
			if (cfg.format == 'images/svg+xml' || cfg.format == 'svg') {
				...
				// PNG
			} else if (cfg.format == 'images/png' || cfg.format == 'png') {
				data = _this.canvas.toDataURL('images/png');
				blob = _this.generateBlob(data, 'images/png');
				...
				if (cfg.output == 'save') {
					...
					location.href = data.replace('images/png', 'images/octet-stream');
				}
				...
				// JPG
			} else if (cfg.format == 'images/jpeg' || cfg.format == 'jpeg' || cfg.format == 'jpg') {
				data = _this.canvas.toDataURL('images/jpeg');
				blob = _this.generateBlob(data, 'images/jpeg');
				...
				if (cfg.output == 'save') {
					...
					location.href = data.replace('images/jpeg', 'images/octet-stream');
				}
			...
			for (var i = 0; i < items.length; i++) {
				...
				if ( _this.cfg.removeImagery ) {
					...
					try {
						...
						datastring = canvas.toDataURL(); // images.src; // canvas.toDataURL(); //
					} catch (err) {
						datastring = image.src; // images.src; // canvas.toDataURL(); //
	...
	*/
	generateOutput: function(cfg, callback) {
		...
		if (cfg.backgroundColor || format == 'images/jpeg') {
		...
		*/
		function drawItWhenItsLoaded() {
			...
			if (_this.processing.buffer.length == _this.processing.drawn) {
				...
				source = new XMLSerializer().serializeToString(buffer[0]); //source = 'data:images/svg+xml;base64,' + btoa();
				offset = buffer[1];
				...
				if (cfg.render == 'browser') {
					...
					source = 'data:images/svg+xml;base64,' + btoa(source);
					...
					if (img.complete || typeof(img.complete) == 'undefined' || img.complete === undefined) {
						...
						img.src = "data:images/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw==";
						img.src = source;
					}
